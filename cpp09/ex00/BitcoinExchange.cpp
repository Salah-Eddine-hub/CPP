...
/*   Updated: 2023/06/11 00:48:14 by sharrach         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int BitcoinExchange::get_values(std::string input)
{
	...
	while (std::getline(infile, line))
	{
		...
		// if (check_date(date) || check_values(value)){
...
bool BitcoinExchange::check_date(std::string date)
{
	...
	if (y == 2009 && m == 1 && d < 2){
		std::cerr << "Error: this date note available => " << date << std::endl;
		return 0;
	}
	if (m == 2 && d > 29){
		std::cerr << "this day not available in this month => " << date << std::endl;
		return 0;
	}
	return 1;
}